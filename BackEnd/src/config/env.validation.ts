...
export function validateProductionEnv(): void {
  ...
  if (oauthClientId && !oauthSecret) {
    throw new Error('GITHUB_OAUTH_CLIENT_SECRET é obrigatório quando GITHUB_OAUTH_CLIENT_ID está definido.');
  }
  if (oauthSecret && !oauthClientId) {
    throw new Error('GITHUB_OAUTH_CLIENT_ID é obrigatório quando GITHUB_OAUTH_CLIENT_SECRET está definido.');
  }

  const frontendUrl = process.env.FRONTEND_URL?.trim();
  if (oauthClientId && !frontendUrl) {
    throw new Error('FRONTEND_URL é obrigatório em produção quando OAuth GitHub está habilitado.');
  }
}
