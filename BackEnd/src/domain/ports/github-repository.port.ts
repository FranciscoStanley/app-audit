...
export interface GitHubRepositoryPort {
  ...
  searchFileInRepo(owner: string, repo: string, filename: string): Promise<boolean>;
  getPackageJson(owner: string, repo: string): Promise<Record<string, unknown> | null>;
  getRequirementsTxt(owner: string, repo: string): Promise<string | null>;
  listWorkflowFiles(owner: string, repo: string): Promise<string[]>;
  getWorkflowContent(owner: string, repo: string, path: string): Promise<string | null>;
}
