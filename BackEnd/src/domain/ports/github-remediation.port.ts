...
export interface GitHubRemediationPort {
  ...
  ensureGitignoreEntry(owner: string, repo: string, entry: string): Promise<void>;
  pinWorkflowActions(owner: string, repo: string, workflowPath: string): Promise<number>;
  ...
  fixDependabotAlert(owner: string, repo: string, alertNumber: number): Promise<void>;
  ...
  createSecurityIssue(owner: string, repo: string, title: string, body: string): Promise<void>;
}
